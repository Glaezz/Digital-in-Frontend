...
function TransactionsPage({authority}){ 
    ...
    const fetchTransactions = async (page = 1) => {
        ...
        try {
            const service = authority === "admin"
                ? adminService.transaction.allTransaction
            ...
            <div className="flex justify-center w-full">
                ...
                <table className="w-full divide-y divide-gray-700">
                    ...
                    {transactions.map(transaction => (
                        <tr 
                            key={transaction.id} 
                            onClick={() => navigate(`${transaction.transaction_id}`)} 
            ...
            <div className="w-full border border-gray-700 rounded-lg">
                ...
                {transactions.map(transaction => (
                    ...
                    <td className="px-4 py-4 text-sm font-medium text-gray-200 flex flex-col">
                        <span>#{transaction.transaction_id}</span>
                        <span className="font-normal text-gray-300">{transaction.time}</span>
                        <span className="font-normal text-gray-300">{transaction.product.detail}</span>
                        <span className="font-normal text-gray-300">{transaction.destination}</span>
                    </td>
                    <td className="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                        <Status status={transaction.status}/>
                    </td>
...
export default function Transactions({authority}){
    return(
        <>
            <HeaderDrawer authority={authority}>
                    <TransactionsPage authority={authority}/>
            </HeaderDrawer>
            <Footer />
        </>
    )
